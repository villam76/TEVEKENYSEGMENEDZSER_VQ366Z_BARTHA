            ...
            submitButton.addEventListener("click", function () {
                ...
                if (daysToComplete !== null) {
                    ...
                    .then(schedule => {
                        ...
                        // Kimaradt feladatok kiszűrése
                        const scheduledNames = schedule.map(task => task.name);
                        const unscheduledTasks = tasks.filter(task => !scheduledNames.includes(task.name));

                        // Figyelmeztetés, ha van kimaradt feladat
                        if (unscheduledTasks.length > 0) {
                            showUnscheduledTasks(unscheduledTasks);
                        }
...
function showUnscheduledTasks(unscheduled) {
    const container = document.getElementById('scheduleTableContainer');

    const warning = document.createElement('div');
    warning.classList.add('alert', 'alert-warning');
    warning.innerHTML = `
        <strong>Figyelem!</strong> A következő feladatokat nem sikerült beosztani (túl kevés rendelkezésre álló idő):
        <ul>
            ${unscheduled.map(t => `<li>${t.name} (${t.duration} óra)</li>`).join('')}
        </ul>
    `;
